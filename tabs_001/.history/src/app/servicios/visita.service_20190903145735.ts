...
export class VisitaService {
  ...
  getVisitaVisitaCheckIn(idInquilino: string) {
    try {
      console.log('Entra a consultar visita idInqquilino==>' + idInquilino);
      const snapshotResult = this.db.collection('sirv_t_visita', ref => ref.where('idUsuario', '==', idInquilino).where('checkOut', '==', ''));
      return snapshotResult;
    } catch (error) {
      console.log('Error al devolver los datos' + error);
    }
  ...
  getVisitaVisitaCheckOut(idInquilino: string) {
    try {
      console.log('Entra a consultar visita idInqquilino==>' + idInquilino);
      const snapshotResult = this.db.collection('sirv_t_visita', ref => ref.where('idUsuario', '==', idInquilino).where('checkIn', '==', '').where('checkIn', '==', ''));
      return snapshotResult;
    } catch (error) {
      console.log('Error al devolver los datos' + error);
    }
