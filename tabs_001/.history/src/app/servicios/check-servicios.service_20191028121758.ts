...
export class CheckServiciosService {
  ...
  async validaFechaCheckServicios(checkServicio: ServiciosInterface) {
    var fechaRegistro: String = checkServicio.fechaCheckin.substring(4, 5); //28-10-2019 10:46:03 
    console.log('validaFechaCheckServicios::'+fechaRegistro);
    try {
      const snapshotResult = this.db.collection('sirv_t_checkServicios').ref.where('idUsuario', '==', checkServicio.idUsuario).orderBy('fechaCheckin', 'desc').limit(1).get().then(resReg => {
        resReg.forEach(async resVisitUnit => {
          const dataVisita: ServiciosInterface = resVisitUnit.data() as ServiciosInterface;
          var fechaRegistrada: String = dataVisita.fechaCheckin.substring(4, 5);
