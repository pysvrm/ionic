...
export class CheckServiciosService {
  ...
  async validaFechaCheckServicios(checkServicio: ServiciosInterface) {
    ...
    try {
      ...
      await this.db.collection('sirv_t_checkServicios').ref.where('idUsuario', '==', checkServicio.idUsuario).where('tipo', '==', checkServicio.tipo).orderBy('fechaCheckin', 'desc').limit(1).get().then(resReg => {
        if (resReg.size > 0) {
