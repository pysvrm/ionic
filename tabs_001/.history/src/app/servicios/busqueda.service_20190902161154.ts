...
export class BusquedaService {
  ...
  addBusquedaInquilino(inquilino: InquilinoInterface){
    ...
    firebase.auth().signInWithEmailAndPassword('vladimir.rodriguez@ferromex.mx', 'p0l1t3cn1c0').catch(function(error) {
      // Handle Errors here.
      var errorCode = error.code;
      var errorMessage = error.message;
      return firebase.database().ref('sirv_c_inquilino').push(inquilino).key;
    });
    
    //console.log('Error al devolver los datos' + error);
    //this.inquilinosCollection.ref.add(inquilino);
    //return this.idUsuarioLocal.key;
