...
export class ConsultaUsuarioInfoPage implements OnInit, OnDestroy{
  ...
  getPicture(){
      ...
      this.fotoTomadaPerfil = this.storage.storage.ref().child('imagesIdentifacion/'
        + ("Jose ArmandoAlvarez Hernandez.jpeg"))
        .getDownloadURL();

        this.fotoTomadaPerfil2 = this.storage.storage.ref('imagesIdentifacion/'
        + ("Jose ArmandoAlvarez Hernandez.jpeg")).getMetadata();

      alert('this.fotoTomadaPerfil'+this.fotoTomadaPerfil);
      alert('this.fotoTomadaPerfil2'+this.fotoTomadaPerfil2) ;
