...
export class ConsultaUsuarioInfoPage implements OnInit, OnDestroy {
  ...
  constructor(public authServices: AuthService,
    ...
    this.fotoIdentifica = "assets/icon/Camera.ico";
  ...
  async getPicture() {
    try {
      await this.storage.storage.ref().child('imagesPerfil/'
        + (this.inquilinoLocal.nombre.trim() + this.inquilinoLocal.apellido.trim() + '.jpeg').trim()
      )
        .getDownloadURL().then(url => {
          this.fotoTomadaPerfil = url;
          alert((this.inquilinoLocal.nombre+" "+ this.inquilinoLocal.apellido.trim() + '.jpeg').trim());
        });
  ...
  async getIdentification(){
    try {
      ...
      + (this.inquilinoLocal.nombre+" "+this.inquilinoLocal.apellido+'.jpeg').trim()
      )
      .getDownloadURL().then(url =>{
          this.fotoTomadaPerfil = url;
          alert((this.inquilinoLocal.nombre.trim()+this.inquilinoLocal.apellido.trim()+'.jpeg').trim());
      });
