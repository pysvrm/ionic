            ...
            this.deptoServ.getBusquedaDepto(this.inquilinoLocal.torre, this.inquilinoLocal.depto)
              .snapshotChanges().subscribe(deptoDetalleRes => {
                deptoDetalleRes.map(async deptoDetalle => {
                  ...
                  console.info("Tipo=>"+this.inquilinoLocal.tipo);

                  if (this.inquilinoLocal.tipo == 'Inquilino') {
                    this.inquilinoLocal.visita = 'I';
                  }
                  else if (this.inquilinoLocal.tipo == 'Visita') {
                    this.inquilinoLocal.visita = 'V';
                  }
                  else if (this.inquilinoLocal.tipo == 'Renta') {
                    this.inquilinoLocal.visita = 'R';
                  } 

                  console.info("Confianza=>"+this.inquilinoLocal.confianza);
                  if (this.inquilinoLocal.confianza == '1') {
                    this.inquilinoLocal.confianza = '1';
                  } else {
                    this.inquilinoLocal.confianza = '0';
                  }
                  ...
                  console.info("Tipo=>"+this.inquilinoLocal.entra);
                  if (this.inquilinoIdLocal.entra == '1') {
                    console.log('Entra a checar fecha y hora');
                    this.visitaLocal.checkIn = ddMMyyyy.toString();
                    this.visitaLocal.checkOut = '0';
                  } else {
                    console.log('No Entra a checar fecha y hora');
