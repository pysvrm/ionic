...
export class RegistroPage implements OnInit, OnDestroy {
  ...
  public inquilinoInterfac = {} as InquilinoInterface;
  ...
  async registraVisitaInquilino() {
    ...
    let miPrimeraPromise = new Promise(async (resolve, reject) => {
      // Llamamos a resolve(...) cuando lo que estabamos haciendo finaliza con éxito, y reject(...) cuando falla.
      // En este ejemplo, usamos setTimeout(...) para simular código asíncrono. 
      // En la vida real, probablemente uses algo como XHR o una API HTML5.
      try {
        const snapshot = await this.busquedaServ.getBusquedaInquilinoNombre(this.inquilinoLocal.nombre, this.inquilinoLocal.apellido);
        const data = snapshot.get().forEach(usuario=>{
          console.log(usuario);
        });
