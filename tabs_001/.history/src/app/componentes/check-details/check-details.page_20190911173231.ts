...
export class CheckDetailsPage implements OnInit, OnDestroy {
  ...
  async checkInVisita() {
    ...
    await this.visitaDepto.getVisitaVisitaCheckIn(this.idInquilino).snapshotChanges().pipe(takeUntil(this.unsubscribe)).subscribe(visitaDetallesCheckIn => {
      visitaDetallesCheckIn.map(visitaDetallesCheckInRes => {
        ...
        if (visitaDetallesCheckIn.length > 0) {
          ...
          if (dataVisita.status == "1" && dataVisita.checkIn != "0") {
            this.checkInAlert();
