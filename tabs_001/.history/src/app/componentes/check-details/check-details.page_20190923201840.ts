        ...
        if (registro > 0) {
          ...
          if (flagSegundoRegistro == true) {
            if (dataVisita.checkIn == '0' && (dataVisita.checkOut == '0')) {
              ...
              this.busquedaServ.getBusquedaInquilinoId(this.idInquilino).snapshotChanges()
                .pipe(takeUntil(this.unsubscribe)).forEach(inquilinoDetalleRes => {
                  this.inquilinoLocal = inquilinoDetalleRes.payload.data() as InquilinoInterface;
                  this.visitaDepto.getVisitaVisita(this.idInquilino).snapshotChanges()
                    .pipe(takeUntil(this.unsubscribe)).subscribe(visitaDetalles => {
                      visitaDetalles.map(deptoDetalle => {
                        const data: VisitaInterface = deptoDetalle.payload.doc.data() as VisitaInterface;
                        this.visitaLocal.id = deptoDetalle.payload.doc.id;
