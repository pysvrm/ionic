...
export class GymPage implements OnDestroy, OnInit {
  ...
  async checkInVisita() {
    ...
    var registro: Number;
    console.log("==Inicio check in GYM==02");
    await this.gymService.getVisitaVisitaGymCheckIn(this.idInquilino).then(async resReg => {
      
      console.log('checkInVisita==>01');
      console.log('checkInVisita==>resReg' + resReg);
      await this.busquedaServ.getBusquedaInquilinoId(this.idInquilino).then(resInquilino => {
        this.inquilinoLocal = resInquilino.data() as InquilinoInterface;
        this.inquilinoLocal.id = resInquilino.id;
      });

      console.log('checkInVisita==>02');
      ...
      if (resReg != undefined) {
        resReg.forEach(async resVisitUnit => {
          console.log('checkInVisita==>resVisitUnit' + resVisitUnit);
          const dataVisita: GymInterface = resVisitUnit.data() as GymInterface;
          
          console.log('checkInVisita==>03');
